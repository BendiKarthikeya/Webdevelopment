...
let std=new Student("sachin",53,"sachin@gmail.com");
console.lot(std);
...
syntax: import {actualName as aliasName....} from '.'


## Code Reusability and Extensibility

### Code Reusablitity 
* It is the process of acceessing code from one module and use it another module.

### Code Extensibility 
* It is the process of extending the module without disturbing the existing features

you can achieve Code Reusability and Extensibility using two techniques
  a) Aggregation [HAS-A]
  b) Inheritance [IS-A]

Aggreation
* It is the process of accessing the members of one lass in another class without configuring any relation b/w the classes.
* It uses Object to Objet communication, It is often termed as "HAS-A" relation.
...
- Multi-versioning enveronment

## Inheritance
* Configuring the relationship b/w classes to access memebers of one class in another class/
...
* A dervied-class constructor must call base-class constructor called.
In amny OOP language  calling parent class constructor from child class is implicit,but in javascript it is expliict
...
if(box.classList.contains("red")) {
  console.log("Box has red class");
} else {
  console.log("Box does not have red class");
}
```
eg#5
...
```javascript
    const box = document.getElementById("box");
    // Toggle the class "red" on the box element  
    box.classList.toggle("red");
```
output : <div class="red" id="box"></div>
...
document.querySelector("button").addEventListner("event")
...
```
this -> sends information about current element
...
BrowserEvents
All javascriptevents are related to browser
Browser events be can configured to any element in a page 
Browser events are classiffied into various categories
  a. Mouse Events
  b. K
...
- `open()` : Opens a file/URL with specified features.  
- `close()` : Closes the current browser window.  
- `print()` : Opens the printer properties dialog.

---

## **location object**
* It provides properties and methods that are used to know the client's location details.

* To track the clint the details javascript as given an object called location which holds the details like browser name , IP address , port number etc

**Members:**
- `host` : Returns the server name or IP address.  
- `protocol` : Returns the protocol (e.g., `http`, `https`, `file`).  
- `port` : Returns the port number.  
- `pathname` : Returns the current file path.  
- `href` : Gets or sets the complete URL.  
- `search` : Returns the query string.  
- `hash` : Returns the current hash location.

# State Management (important concept)

## a. Web application uses a protocol called "http".

Example:

* `<img src="public/images/...">` ✅ (virtual path, forward slash — only this works)
* `<img src="D:\images\...">` ❌ (physical path — doesn't work in real-time)

---

## b. Protocol

Protocol defines rules which are followed by computers in a network to communicate with each other.
...
1. Server allocates memory for a request made by the client, processes the request, and sends the response to the client.
   After sending the response, it cleans up the client-related information from memory (**stateless**).
2. Stateless nature of the server is good for memory management, but it is **not good for continuous operations** from the client.
3. Every web application requires state management techniques so that client-related information can be saved and accessed across requests.
4. State management can be on two sides: **client-side** | **server-side**.
5. Examples:

   * Login to an application in **my laptop** → browser remembers your credentials.
   * Login to an application in **friend's laptop** → you need to enter credentials again.
   * E-commerce website → payment page → card details entered:

     * **In my laptop** → server remembers the details.
     * **In friend's laptop** → details not remembered.

---

## d. State management techniques are classified into 2 types:
...
It allows storing client details on the client device.
...
* A value appended into the URL and stored in the browser address bar.
* Comprises of **key** and **value** appended using `?`.
  Example:

  ```
  ?key=value&key=value
  ```
* Can carry data from one page to another.
* **Limitations:**

  * Not safe.
  * Cannot submit complex data.
  * Data limit: 20,248 characters.
* To access the query string, we use **BOM**:
...
**Q:** How to navigate dynamically in JavaScript?
**A:** By using `location.href`
...
* `appName` : Returns the browser family name.
* `appVersion` : Returns the browser version.
* `userAgent` : Returns browser supported platforms and details.
* `platform` : Returns the current platform.
* `language` : Returns the browser's current language.
* `cookieEnabled` : Returns the cookie status.
* `plugins[]` : Returns the collection of all installed plugins.
* `mimeTypes[]` : Returns all MIME types supported.
* `geoLocation()` : Returns the current geo-location of the client.


#### **Example #1: Cookie, Browser, Platform, and Language**

```html
<script type="text/javascript">
    document.cookie = "test=cookieEnabled";
    let cookieEnabled = document.cookie.indexOf("test=cookieEnable") !== -1;
    document.write(`
        Your Browser Version : ${navigator.appVersion} <br>
        Cookies : ${cookieEnabled ? "You can login " : "Enable cookies"} <br>
...
```
Your Browser Version : 5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/134.0.0.0 Safari/537.36
Cookies : You can login
Platform : Win32
Language : en-US
```
...
It is used to access current browsing history details.

### Members:

* `length` : Gets the total count of pages in the current browsing history.
* `forward()` : Moves to the next page in browsing history.
* `back()` : Moves to the previous page in browsing history.
* `go()` : Moves to a specific page in history.

---

### **Example #1: Browsing History Count**

```html
<script type="text/javascript">
    var now = new Date();
    document.write(`
        You opened page at : ${now.toLocaleDateString()} <br>
        No of pages visited is : ${history.length}
    `)
</script>
```
...
```
You opened page at : 21/3/2025
No of pages visited is : 5
```
...
It provides all the properties and methods to handle elements in the document (inside `<body>`).
...
<img src="./Images/stateManagement.png">
...
- Cannot submit complex data.
- Data limit: 20,248 characters.
- To access the query string, we use **BOM**:  `location.search`
...
<body>
    <form method="get" action="process.html">
        <dl>
           <h3>Login page</h3>
           <dt>Username</dt>
           <dd>
...
<head>
    ...
    <script type="text/javascript">
        function processLogic() {
              ...
              let result = username.substring(username.indexOf("=") + 1);
              document.querySelector("p").innerHTML = "Hello " + result.bold().fontsize(20);
        }
    </script>
</head>
<body onload="processLogic()">
        <h3> User Details </h3>
        <p></p>
</body>
</html>
...
* Local Storage is a browser memory location.
* It is permanent until manually deleted.
* Accessible across browser tabs.
* Available after closing the browser or shutting down the device.
...
```javascript
localStorage.setItem("k", "V"); // Store value
localStorage.getItem("k");       // Retrieve value
localStorage.removeItem("k");    // Remove value
```

### **index.html**
> <a href="../BOM/localstrorage/index.html">Click on this</a>

### **home.html**
> <a href="../BOM/localstrorage/process.html">Click on this</a>

## Session Storage

- It is the memory of the browser allocated for the client.
- It is temporary memory.
- It is accessible only to the current tab.
- It is removed when the browser is closed.

**Syntax:**
```javascript
sessionStorage.setItem("k", "v");   // Store value
sessionStorage.getItem("k");        // Retrieve value
sessionStorage.removeItem("k");     // Remove value
```

### **index.html**
> <a href="../BOM/session/index.html">Click on this</a>

### **home.html**
> <a href="../BOM/session/home.html">Click on this</a>

## Cookies

Cookie is a temporary file created by the server on the client side.

Cookies are classified into 2 types:
1. **Non-persistent cookie**: Created without an expiry date; once the browser is closed, it expires automatically.
2. **Persistent cookie**: Created with an expiry date; remains available even if the browser is closed until the expiry time.

   
## **Note:** 
Just by closing the browser window the cookie would be deleted [non-persistence]
To delete after some interval we set 'expires=Thu, 01 Jan 1970 00:00:00 UTC;'|'max-age=0;' 
